  ...
  class Alert
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PagerDutyOpenapiClient::Alert` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PagerDutyOpenapiClient::Alert`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'alert_key')
        self.alert_key = attributes[:'alert_key']
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["alert"])
      return false unless type_validator.valid?(@type)
      status_validator = EnumAttributeValidator.new('String', ["triggered", "resolved"])
      return false unless status_validator.valid?(@status)
      severity_validator = EnumAttributeValidator.new('String', ["info", "warning", "error", "critical"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["alert"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["triggered", "resolved"])
    ...
    def severity=(severity)
      validator = EnumAttributeValidator.new('String', ["info", "warning", "error", "critical"])
    ...
    def hash
      [created_at, type, status, alert_key, service, first_trigger_log_entry, incident, suppressed, severity, integration, body].hash
    end
