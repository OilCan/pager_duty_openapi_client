  ...
  class ExtensionSchema
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PagerDutyOpenapiClient::ExtensionSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PagerDutyOpenapiClient::ExtensionSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'guide_url')
        self.guide_url = attributes[:'guide_url']
      end

      if attributes.key?(:'send_types')
        if (value = attributes[:'send_types']).is_a?(Array)
    ...
    def hash
      [icon_url, logo_url, label, key, description, guide_url, send_types, url].hash
    end
