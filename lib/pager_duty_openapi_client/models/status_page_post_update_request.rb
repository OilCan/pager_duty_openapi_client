...
module PagerDutyOpenapiClient
  ...
  class StatusPagePostUpdateRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PagerDutyOpenapiClient::StatusPagePostUpdateRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PagerDutyOpenapiClient::StatusPagePostUpdateRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @impacted_services.nil?
        invalid_properties.push('invalid value for "impacted_services", impacted_services cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @impacted_services.nil?
      return false if @impacted_services.length < 0
    ...
    def hash
      [_self, post, message, status, severity, impacted_services, update_frequency_ms, notify_subscribers, reported_at, type].hash
    end
