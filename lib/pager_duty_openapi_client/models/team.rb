  ...
  class Team
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PagerDutyOpenapiClient::Team` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PagerDutyOpenapiClient::Team`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["team"])
      return false unless type_validator.valid?(@type)
      return false if @name.nil?
      return false if @name.to_s.length > 100
      return false if !@description.nil? && @description.to_s.length > 1024
      default_role_validator = EnumAttributeValidator.new('String', ["manager", "none"])
    ...
    def default_role=(default_role)
      validator = EnumAttributeValidator.new('String', ["manager", "none"])
    ...
    def hash
      [type, name, description, default_role].hash
    end
