...
module PagerDutyOpenapiClient
  class ListIncidentRelatedChangeEvents200ResponseChangeEventsInner
    ...
    attr_accessor :correlation_reason
    ...
    def self.openapi_types
      {
        ...
        :'correlation_reason' => :'ListIncidentRelatedChangeEvents200ResponseChangeEventsInnerAllOfCorrelationReason'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PagerDutyOpenapiClient::ListIncidentRelatedChangeEvents200ResponseChangeEventsInner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PagerDutyOpenapiClient::ListIncidentRelatedChangeEvents200ResponseChangeEventsInner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'correlation_reason')
        self.correlation_reason = attributes[:'correlation_reason']
      end
    end
    ...
    def hash
      [timestamp, type, services, integration, routing_key, summary, source, links, images, custom_details, correlation_reason].hash
    end
