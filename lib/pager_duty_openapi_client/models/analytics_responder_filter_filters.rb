...
module PagerDutyOpenapiClient
  ...
  class AnalyticsResponderFilterFilters
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PagerDutyOpenapiClient::AnalyticsResponderFilterFilters` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PagerDutyOpenapiClient::AnalyticsResponderFilterFilters`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'date_range_start')
        self.date_range_start = attributes[:'date_range_start']
      end
      ...
      if attributes.key?(:'responder_ids')
        if (value = attributes[:'responder_ids']).is_a?(Array)
          self.responder_ids = value
        end
      end
    ...
    def hash
      [date_range_start, date_range_end, urgency, team_ids, responder_ids, priority_ids, priority_names].hash
    end
