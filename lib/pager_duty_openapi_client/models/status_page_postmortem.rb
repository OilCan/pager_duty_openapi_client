  ...
  class StatusPagePostmortem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PagerDutyOpenapiClient::StatusPagePostmortem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PagerDutyOpenapiClient::StatusPagePostmortem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@message.nil? && @message.to_s.length > 10000
    ...
    def valid?
      ...
      return false if !@message.nil? && @message.to_s.length > 10000
      true
    end
    ...
    def hash
      [id, _self, post, message, notify_subscribers, reported_at, type].hash
    end
